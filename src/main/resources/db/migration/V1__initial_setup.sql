...
insert into user_roles(user_id, role_name) select u.id, 'ROLE_ADMIN' from users u where u.email='admin@syntaxerror.blog';
insert into user_roles(user_id, role_name) select u.id, 'ROLE_USER' from users u where u.email='admin@syntaxerror.blog';
